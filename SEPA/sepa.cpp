...
/*  
A Simple Efficient Permutation Algorithm (SEPA) Algorithm to find all permutation of a string 
Time Complexity: O(n!)
*/
...
int permute(std::string &str, int len)
{
    ...
    /* 
        variables len and key are used to walk through the tail, exchanging pairs from both ends of the tail.
        len and key are reused to save memory
    */
...
int main()
{
    ...
    /* A short test loop to print each permutation, which are created in sorted order. */
    do {
        std::cout << s << "\n";
    } while(permute(s, 10));
